...
class OrganizationEntity(EntityBase):
    ...
    # Whether the organization is accepting members
    accepting_members: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=True
    )
    # Whether an application is required for membership
    application: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    ...
    members: Mapped[list[MemberEntity]] = relationship(
        back_populates="organization", cascade="all,delete"
    )
    # Access users through the member association
    users: Mapped[list["UserEntity"]] = relationship(
        secondary="members", back_populates="organizations"
    )
    ...
    def from_model(cls, model: Organization) -> Self:
        ...
        return cls(
            ...
            public=model.public,
            accepting_members=model.accepting_members,
